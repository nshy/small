...
region_test_join()
{
	...
	for (int i = 0; i < 217; i++) {
		...
		char *ptr = (char *)region_join(&region, total);
		fail_unless(ptr != NULL);
		for (int j = 0; j < n; j++) {
...
region_test_alignment()
{
	...
	for (int i = 0; i < 9999; i++) {
		...
		if (s == 0) {
			...
			void *ptr = region_aligned_alloc(&region, rand() % 333,
							 alignment);
			fail_unless(ptr != NULL);
...
region_test_poison()
{
	...
	for (int i = 0; i < 87; i++) {
		size_t size_r = rand() % size_max;
		size_t size_a = rand() % size_r;
		char *ptr_r = (char *)region_reserve(&region, size_r);
		fail_unless(ptr_r != NULL);
		memset(ptr_r, 0, size_a);
		char *ptr_a = (char *)region_alloc(&region, size_a);
...
region_test_tiny_reserve_size()
{
	...
	struct region region;
	region_create(&region, &cache);
	size_t size_max = small_getpagesize();
	for (int i = 0; i < 87; i++) {
		size_t size = rand() % (size_max + 1);
...
static void
region_test_zero_alloc_truncation()
{
	plan(1);
	header();
	...
	region_create(&region, &cache);
	for (int i = 0; i < 99; i++) {
		region_alloc(&region, rand() % 7000);
		struct rlist *last = region.allocations.prev;
		size_t used = region_used(&region);
		int alloc_size = rand() % 11;
		for (int j = 0; j < alloc_size; j++) {
			void *base = region_alloc(&region, 0);
			fail_unless(base != NULL);
			fail_unless(region_used(&region) == used);
		}
		region_truncate(&region, used);
		fail_unless(region_used(&region) == used);
		fail_unless(region.allocations.prev == last);
	}
	region_destroy(&region);
	ok(true);
